...
Clusterer <- R6Class( # nolint
  ...
  public = list(
    ...
    calc_silhouette_scores = function() {
      ...
      for (resolution_index in seq_len(num_resolutions)) {
        for (subsample_index in seq_len(num_subsamples)) {
          ...
          sample_size <- min(1000, nrow(pca_matrix))
          idx <- sample(seq_len(nrow(pca_matrix)), sample_size)
    ...
    set_best_clusters = function(best_resolution) {
      cluster_column_name <-
        paste("RNA_snn_res.", best_resolution, sep = "")
    ...
    #FOR CITESEQ CHANGE THE RNA BELOW TO ADT YERRRR
    
     find_top_genes = function(assay_name = "RNA", n_top_genes = 5,
